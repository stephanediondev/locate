...
var icon_marker = new L.Icon.Default({iconUrl: '../../../../images/locatemarker.png'});
var icon_event = new L.Icon.Default({iconUrl: '../../../../images/locatemarker_event.png'});
...
function map_init() {
    ...
    if('https:' == document.location.protocol) {
        ...
        urlTemplate = 'api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}';
    }
    L.tileLayer(urlTemplate, {
        'maxZoom': 18,
        'tileSize': 512,
	'zoomOffset': -1,
        'errorTileUrl': '//locatemarker.com/app/icons/icon-256x256.png',
...
function map_setcircles() {
    ...
    for(i=0;i<total_markers;i++) {
        latitude = map_markers[i]._latlng.lat;
        longitude = map_markers[i]._latlng.lng;
...
function map_addmarker(markerdata) {
    if(fb_places[markerdata.id] == null) {
        options= {};
        if(markerdata.icon == 'event') {
            options.icon = icon_event;
        } else if(markerdata.icon == 'marker') {
    ...
    if(navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            function(position) {
                ...
                if(!marker_me) {
                    content = '<p class="intro"><strong>' + user.name + '</strong></p>';
                    if(user.picture) {
                    ...
                    marker_me = new L.marker([position.coords.latitude, position.coords.longitude], {'title': user.name});
...
if (typeof response.data[i].location !== 'undefined') {
			var buttons = [];
                        var score = 0;
...
function fb_permissions() {
    ...
    FB.api(facebook_api_prefix + '/me/permissions', 'get', {}, function(response) {
        ...
        for(i in permissions_recommended) {
...
$(document).ready(function() {
    ...
    $('.map_locateme').bind('click', function(event) {
        event.preventDefault();
        center_on_me = 1;
        map_locateme();
        map_setcircles();
    ...
    $('.fb_login').bind('click', function(event) {
        event.preventDefault();
    ...
    $('.fb_logout').bind('click', function(event) {
        event.preventDefault();
        ui_sethash();
        fb_logout();
    });
